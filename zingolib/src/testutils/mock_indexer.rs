...
//! feature, to downstream test crates (e.g. libtonode-tests).
...
fn orchard_coinbase_bundle(
    ...
    .expect("coinbase flags are representable");
    builder
        .add_output(
            None,
            recipient,
            orchard::value::NoteValue::from_raw(reward.into_u64()),
            [0; 512],
        )
        .expect("coinbase bundles take outputs");
    let (unauthorized, _) = builder
        .build::<ZatBalance>(rand::rngs::OsRng)
        .expect("a one-output coinbase bundle builds")
        .expect("a bundle with an output is non-empty");
...
impl MockChain {
    ...
    /// Returns the activation schedule the chain was created with.
    pub fn activation_heights(&self) -> ActivationHeights {
        match self.chain_type {
            ...
            ChainType::Mainnet | ChainType::Testnet => {
                unreachable!("the mock chain is always regtest")
            }
        }
    }
    ...
    async fn send_transaction(
        ...
        if chain.reject_all_sends {
            ...
            return Err(Status::internal(
                "mock fault: this indexer suppresses every transaction",
            ));
        }
        if let Some(error_code) = chain.answer_sends_with_error_code {
            ...
            return Ok(Response::new(SendResponse {
                error_code,
                error_message: "mock fault: this indexer rejects the transaction".to_string(),
            }));
        }
...
impl MockNet {
    ...
    async fn serve(chain: MockChain) -> Self {
        ...
        let indexer_uri: http::Uri = format!("http://127.0.0.1:{}", addr.port())
            .parse()
    ...
    /// Returns the activation schedule of the served chain.
    pub fn activation_heights(&self) -> ActivationHeights {
        match self.chain_type {
            ChainType::Regtest(activation_heights) => activation_heights,
            ChainType::Mainnet | ChainType::Testnet => {
    ...
    pub async fn chain_height(&self) -> u32 {
        self.chain.read().await.tip()
    }

    /// Mines `n` blocks, the first one carrying the mempool, each one
    /// paying the miner address when one is set.
    pub async fn generate_blocks(&self, n: u32) {
        for _ in 0..n {
            let Some(miner) = &self.miner_address else {
    ...
    pub async fn client(&mut self, mnemonic: &str) -> LightClient {
        let config = self
            ...
            .build();
        let mut lightclient = LightClient::new(config, true, None)
            .await
    ...
    pub async fn client_from_file(&mut self, client_index: usize) -> LightClient {
        ...
        let config = ClientConfig::builder()
            ...
            .build();
        LightClient::new(config, true, None)
            .await
            .expect("mock-net client construction succeeds")
    }
}
...
impl ConductChain for MockNet {
    ...
    async fn create_faucet(&mut self) -> LightClient {
        ...
        let activation_heights = self.chain.read().await.activation_heights();
        let funding = faucet_funding_transaction_for(
            activation_heights,
            vec![(&address, FAUCET_FUNDING, None)],
        )
        .await;
...
pub async fn faucet_funding_transaction(receivers: Vec<(&str, u64, Option<&str>)>) -> Vec<u8> {
    faucet_funding_transaction_for(ActivationHeights::default(), receivers).await
}

/// As [`faucet_funding_transaction`], for a chain with its own activation
/// schedule. The faucet spends an Ironwood note where NU6.3 is active at its
/// target height and an Orchard note where it is not, so the transaction
/// carries the consensus branch and pools that chain expects.
pub async fn faucet_funding_transaction_for(
        ...
        .is_nu_active(NetworkUpgrade::Nu6_3, BlockHeight::from_u32(FAUCET_TIP + 1))
    {
        ...
        faucet.orchard_note(total + FAUCET_HEADROOM)
    };
    let mut wallet = faucet.build();
    let request = from_inputs::transaction_request_from_send_inputs(receivers)
        .expect("the funding receivers form a valid request");
    let proposal = wallet
